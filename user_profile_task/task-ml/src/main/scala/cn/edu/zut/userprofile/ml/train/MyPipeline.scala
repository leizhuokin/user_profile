package cn.edu.zut.userprofile.ml.train
...
class MyPipeline {
  ...
  def  printEvaluate(predcitedDataFrame: DataFrame): Unit ={
    val predictionLabelRDD: RDD[(Double, Double)] = predcitedDataFrame.rdd.map { row =>
      (row.getAs[Double]("prediction_col"), row.getAs[Double]("label_index"))
    }
